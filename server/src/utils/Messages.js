export const MESSAGES = {
  SESSION_STARTED: "Session started",
  SESSION_REMOVED: "Session removed successfully",
  ERROR_REMOVING_SESSION: "Error removing session",
  REFRESH_TOKEN: "Refresh token issued successfully",
  REFRESH_TOKEN_EXPIRED: "Refresh token expired",
  REFRESH_TOKEN_INVALID: "Refresh token is invalid",
  REFRESH_TOKEN_ERROR: "Error refreshing token",
  MISSING_FIELDS: "Missing fields",
  ACCESS_DENIED: "Access denied",
  USER_UNVERIFIED: "User unverified",
  USER_NOT_FOUND: "User not found",

  // Sign-up related messages
  USER_ALREADY_EXISTS: "User already exists",
  SIGNUP_SUCCESS: "User signed up successfully",
  SIGNUP_ERROR: "Error during sign-up",
  PASSWORDS_DO_NOT_MATCH: "Passwords do not match",

  // Login related messages
  INVALID_CREDENTIALS: "Invalid email or password",
  LOGIN_SUCCESS: "User logged in successfully",
  LOGIN_ERROR: "Error during login",
  LOGOUT_SUCCESS: "User logged out successfully",
  LOGOUT_ERROR: "Error during logout",

  // whatsapp related messages
  CLIENT_NOT_FOUND: "Client not found",
  WHATSAPP_MESSAGE_SENT: "WhatsApp message sent successfully",
  WHATSAPP_MESSAGE_ERROR: "Error sending WhatsApp message",

  // socket related messages
  SOCKET_CONNECTION_ERROR: "Error connecting to socket",
  WHATSAPP_CONNECTED: "WhatsApp connected",
  SESSIONID_IS_REQUIRED: "Session ID is required",

  // contact related messages
  ADDCONTACTERROR: "Error adding contact",
  CONTACTEXIST: "Contact with this phone number already exists",
  ADDCONTACTSUCCESS: "Contact added successfully",
  UPDATECONTACTERROR: "Error updating contact",
  UPDATECONTACTSUCCESS: "Contact updated successfully",
  DELETECONTACTERROR: "Error deleting contact",
  CONTACTNOTEXIST: "Contact not found",
  DELETECONTACTSUCCESS: "Contact deleted successfully",
  GETCONTACTERROR: "Error fetching contacts",
  GETCONTACTSUCCESS: "Contacts fetched successfully",

  // default message related messages
  ADDDEFAULTMESSAGEERROR: "Error adding default message",
  ADDDEFAULTMESSAGESUCCESS: "Default message added successfully",
  GETDEFAULTMESSAGESUCCESS: "Default messages fetched successfully",
  GETDEFAULTMESSAGEERROR: "Error fetching default messages",
  UPDATEDEFAULTMESSAGEERROR: "Error updating default message",
  UPDATEDEFAULTMESSAGESUCCESS: "Default message updated successfully",
  DELETEDEFAULTMESSAGEERROR: "Error deleting default message",
  DELETEDEFAULTMESSAGESUCCESS: "Default message deleted successfully",
  DEFAULTMESSAGENOTFOUND: "Default message not found",
};
