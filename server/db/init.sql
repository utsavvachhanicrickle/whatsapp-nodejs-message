...
AND NOT EXISTS (
    SELECT 1
    FROM information_schema.tables
    WHERE table_name = 'personal_messages'
) THEN
ALTER TABLE chat_messages
    RENAME TO personal_messages;
END IF;
END $$;
...
ADD COLUMN "personalMessageId" UUID REFERENCES personal_messages(_id) ON DELETE CASCADE;
...
ALTER TABLE contacts
ADD CONSTRAINT contacts_phone_user_session_key UNIQUE ("phoneNumber", "userId", "sessionId");
END IF;
END $$;